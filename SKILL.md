...
**wemedia = 自媒体运营（策略层），media-tools = 自媒体执行（工具层）**
...
**main 的角色**：编排中心。wemedia 输出内容 + 执行指令 → main 调用 media-tools 完成发布。
...
**本质**：wemedia = 自媒体运营（策略层），media-tools = 自媒体执行（工具层）。
...
main（Step 7.5）
    → 读取 wemedia 交付物
    → 调用 media-tools 的 publish_pipeline.py 发布
    ↓
...
**main 的职责**：编排调度，从 wemedia 接收内容 + 向 media-tools 下发执行指令。
